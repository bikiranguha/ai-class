...
def makeNeighboursDFS(parent,parentID):
	zIndex = parent.index(0)
...
class stateDFS(list):
    ...
    def __init__(self, parentState,parentID):
    	self.parentState = parentState
    	self.nbr = makeNeighboursDFS(parentState,parentID)
...
#	exploredID = []
	...
	while len(frontier) > 0: # until frontier is empty
		...
		currentState = stateDFS(currentList,currentID)
		currentTuple=tuple(currentList)
		explored.add(currentTuple)
#		exploredID.append(currentID)
...
#		os.system("pause")
		...
		if currentList == [0,1,2,3,4,5,6,7,8]:
			print "path: ",currentID

		else:
			if currentState.nbr.List:
				for neighbour in currentState.nbr.List:
					if neighbour not in frontier:
						neighbourTupl =tuple(neighbour)
						if neighbourTupl not in explored:
